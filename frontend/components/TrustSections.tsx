import { AUTHORITY_PILLARS, COD_STEPS, TRUST_BAR } from "@/data/brand";
...
export function AuthorityGrid() {
  return (
    <section className="mx-auto max-w-container px-4 py-14">
      <h2 className="text-2xl font-bold">علاش سفراسكين؟</h2>
  ...
  return (
    <section className="bg-ink text-cream">
      <div className="mx-auto max-w-container px-4 py-14">
        <h2 className="text-2xl font-bold">كيفاش كيمشي الطلب؟</h2>
        <p className="mt-2 max-w-xl text-sm leading-7 text-cream/70">
          ساهل بحال ما كيكونش معقّد. ثلاثة خطوات، والفلوس غير ملي توصّل الطلبيّة.
        </p>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {COD_STEPS.map((step) => (
            <article key={step.step} className="rounded-2xl border border-white/10 bg-white/5 p-5">
              <span className="text-saffron font-english text-sm">0{step.step}</span>
